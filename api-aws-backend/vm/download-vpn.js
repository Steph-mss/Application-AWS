...
router.get("/download-vpn/:user_id/:vm_id", async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            "SELECT vm_dir FROM vms WHERE id = $1 AND user_id = $2 AND expires_at > NOW() LIMIT 1",
            [vm_id, user_id]
        );
        ...
        const { vm_dir } = result.rows[0];

        // Construire le chemin du fichier VPN
        const vpnConfigPath = path.join(__dirname, `../${vm_dir}/client.ovpn`);
        ...
        res.download(vpnConfigPath, `${vm_dir}.ovpn`);
    } catch (err) {
